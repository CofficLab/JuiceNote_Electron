    ...
    <div class="dropdown-end dropdown-hover dropdown">
      ...
      <ul tabindex="0" class="dropdown-content z-50 flex w-12 flex-col gap-4 rounded-2xl bg-base-100 py-2 shadow">
        <li v-if="isRouteLesson">
          <Delete :showText="false" class="btn-ghost btn-sm btn w-12"></Delete>
        </li>
        <li v-if="isRouteLesson">
          <CreateChild :showText="false" class="btn-ghost btn-sm btn w-12"></CreateChild>
        </li>
        <li>
          <Edit :showText="false" class="btn-ghost btn-sm btn w-12"></Edit>
        </li>
        <li v-if="isRouteLesson">
          <Copy :showText="false" class="btn-ghost btn-sm btn w-12"></Copy>
        </li>
        <li>
          <Commit :showText="false" class="btn-ghost btn-sm btn w-12"></Commit>
        </li>
        <li v-if="isRouteLesson">
          <Visible :showText="false" :node="book" class="btn-ghost btn-sm btn w-12"></Visible>
        </li>
      </ul>
    </div>
  </div>
